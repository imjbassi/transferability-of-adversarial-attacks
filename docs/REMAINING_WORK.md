...
Status at 25 September 2026, after completing and freezing the repaired first pass for all 80 ranks. The manuscript is **not publication-ready**.
...
- All 80 first-pass evidence reviews complete and validated; none partial or pending. A pre-freeze consistency sweep downgraded rank 76's field-3 group codes to unclear. The repaired sheet, reliability-format first pass and stratified summary are exported and frozen (`survey/evidence_audit/freeze_manifest.json`), and a blind-recode packet for the preregistered 16-paper sample is in `survey/blind_recode/`. All counts remain non-blind and single-auditor.

## Required before defensible survey claims

1. **Evidence reviews (done for first pass):** all 80 papers have documented reviews and a consistency sweep was run before freezing; ranks 47, 53 and 78 remain flagged borderline in their reviews. The original task was to review the remaining papers, including supplements, all result scopes, six judgments, evidence locators, applicability, artifact checks and bounds eligibility. Preserve unclear versus explicit no and all positive conditioning evidence. Keep the original corpus/scheme/tag immutable; the September 23 amendment is retrospective.
2. **Export and freeze the repaired first pass (done 25 September 2026):** `python survey/build_repaired_coding_sheet.py --check` and `python survey/freeze_first_pass.py --check` reproduce and verify the frozen outputs. Mixed explicit, non-applicable and unresolved strata are reported separately in `repaired_summary.json`. Any later change needs a dated amendment and a new freeze. The manuscript still uses the provisional historical numbers and must be updated only after reliability data exist.
3. **Obtain genuinely blind reliability data (blocked on an independent person):** the preregistered sample (seed 20260919) and a judgment-free packet are ready in `survey/blind_recode/`. An independent recoder must recode without first-pass access, and compute Cohen's kappa per field with counts and uncertainty/limitations. The previous copied recode is invalid. This context has seen the first pass and cannot act as the blind recoder. The author waived waiting, not independence. Any kappa below 0.7 requires a documented tightening pass and full recode under the revised scheme.
...
The repaired first pass is frozen, but no new valid kappa, final corpus prevalence, corpus-wide bounds distribution or full-paper replication claim is available at this stopping point.
