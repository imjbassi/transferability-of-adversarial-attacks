...
Separate clean source/target accuracy was explicit in 5/80 papers (6.25\%); clean-correct transfer eligibility in 7/80 (8.75\%); source-success conditioning in 1/80 (1.25\%); and absolute transfer counts in 0/80, with 57 percentage-only evaluations coded no and 23 unclear. A numerical $L_\infty$ or $L_2$ budget was explicit in 46/80 (57.5\%). Five papers (6.25\%) met the artifact rule. These rates are descriptive of the preregistered citation-and-text-filtered corpus, which intentionally retains tangential matches and surveys rather than applying a post hoc topical exclusion.

\section{Recomputation and analytic bounds}
The five qualifying artifact releases were checked out at recorded commit hashes. Their code and model links make a fresh execution possible, but none supplies per-example clean/source/target predictions in the released repository; exact PTR, CTR, $a_{st}$, and $b_{st}$ therefore require rerunning legacy attack stacks and adapting their aggregate evaluators. We report this distinction as a finding in its own right: artifact availability was 6.25\%, while immediately reusable per-example availability was 0/80. The recomputation ledger records each checkout and execution result without describing any paper as erroneous.

We completed one such end-to-end adaptation on the released 1,000-image evaluation set, retaining clean and attacked predictions for the source and nine targets. For the six targets with aligned Table~1 comparators, recomputed unconditional attacked-error rates differed by $-2.0$ to $+0.7$ percentage points. Pairwise-clean-correct PTR ranged from 45.69\% to 99.68\%, CTR from 45.84\% to 100\%, and $a_{st}$ from 99.67\% to 99.68\%; $b_{st}$ was 0 in each direction, with only three failed-source examples per eligible set. Four other legacy workflows were checked out and audited but not completed; their metric cells remain blank rather than estimated.
