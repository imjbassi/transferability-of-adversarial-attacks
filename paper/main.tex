...
\hypersetup{pdftitle={What transfer rates do not tell you: conditioning practice in adversarial transfer evaluation},pdfauthor={Jaiveer Bassi}}
...
The corpus was frozen before coding at repository tag \texttt{survey-preregistration}. We queried Semantic Scholar for papers citing Liu et al. (arXiv:1611.02770) or Papernot et al. (arXiv:1602.02697), retained publication years 2018--2026 whose title or abstract contained ``transfer'' plus ``adversarial,'' ``black-box,'' or ``surrogate,'' sorted by citation count, and kept the first 80. The query, UTC run time, complete paginated response, and ranked list are archived with the paper.

Each paper and linked supplement was coded yes/no/unclear for six preregistered fields: separate source and target clean accuracy; exclusion of clean target errors from the transfer denominator; conditioning on source attack success; absolute numerator and denominator counts; a numerical $L_\infty$ or $L_2$ budget; and artifacts sufficient to recompute, defined as code plus required checkpoints or per-example outputs. Silence is unclear. In particular, ``evaluated on correctly classified images'' is yes for clean eligibility. Evidence locations are recorded in \texttt{survey/coding\_sheet.csv}.

The planned seven-day separation was waived by explicit instruction after preregistration. We therefore report an immediate, seeded 16-paper repeatability check rather than a delayed stability estimate. The sample seed, ranks, second-pass sheet, and computation are archived; all per-field and pooled Cohen's $\kappa$ values are 1.00. This perfect value should be interpreted in light of the zero-day interval and common coder.

\section{Survey results}
Separate clean source/target accuracy was explicit in 5/80 papers (6.25\%); clean-correct transfer eligibility in 7/80 (8.75\%); source-success conditioning in 1/80 (1.25\%); and absolute transfer counts in 0/80, with 57 percentage-only evaluations coded no and 23 unclear. A numerical $L_\infty$ or $L_2$ budget was explicit in 46/80 (57.5\%). Three papers (3.75\%) met the artifact rule. These rates are descriptive of the preregistered citation-and-text-filtered corpus, which intentionally retains tangential matches and surveys rather than applying a post hoc topical exclusion. An artifact audit records two initially plausible releases as unclear because the available checkpoints or evaluation path were not sufficient to reconstruct the surveyed transfer results; this applies the preregistered sufficiency rule rather than excluding papers from the corpus.

\section{Recomputation and analytic bounds}
The three qualifying artifact releases were checked out at recorded commit hashes and rerun on their released 1,000-image evaluation set. None supplies per-example clean/source/target predictions in the released repository, so we retained those predictions by adapting the aggregate evaluators; for the two TensorFlow~1 releases, an equation-preserving PyTorch adapter used the authors' linked converted checkpoints. We report this distinction as a finding in its own right: artifact availability was 3.75\%, while immediately reusable per-example availability was 0/80. The recomputation ledger records each execution result without describing any paper as erroneous.

All three qualifying releases were recomputed, yielding 27 source--target summaries and per-example prediction files. For the 20 entries with aligned published comparators, recomputed unconditional attacked-error rates differed by $-3.9$ to $+5.5$ percentage points (median $-0.65$). Pairwise-clean-correct PTR ranged from 14.64\% to 100\%, CTR from 14.68\% to 100\%, and $a_{st}$ from 99.66\% to 100\%. The failed-source stratum was empty in 18 directions, making $b_{st}$ undefined rather than zero; in the nine directions where it was nonempty, $b_{st}=0$. The deltas are descriptive portability results, not claims of error in the published studies.

For a paper reporting target clean error $e_t$ and unconditional attacked error $u_t$, Equation~1 implies $u_t-e_t\leq(1-e_t)q$ and hence $\max(0,u_t-e_t)$ lower-bounds the all-input fraction of newly induced errors. One paper supplied aligned clean and unconditional attacked accuracies at the required granularity. Across its 96 off-diagonal source--target--attack rates, the bound ranges from 1.2 to 77.9 percentage points, with median 34.5, mean 35.33, and interquartile range 23.55--45.35. Per-rate values are in \texttt{survey/analytic\_bounds.csv}.
