...
Separate clean source/target accuracy was explicit in 5/80 papers (6.25\%); clean-correct transfer eligibility in 7/80 (8.75\%); source-success conditioning in 1/80 (1.25\%); and absolute counts in 0/80, with 57 percentage-only evaluations coded no and 23 unclear. A numerical norm budget was explicit in 46/80 (57.5\%). Five papers (6.25\%) met the artifact rule. These rates describe the preregistered corpus, including tangential mechanical-filter matches retained to avoid post hoc exclusion.

\section{Recomputation and analytic bounds}
The five qualifying releases were checked out at recorded commits. None supplies per-example clean/source/target predictions directly; their legacy attack stacks must be rerun and adapted beyond aggregate success-rate evaluators. Artifact availability (6.25\%) and immediately reusable per-example availability (0/80) are therefore distinct findings. For aligned target clean error $e_t$ and unconditional attacked error $u_t$, $\max(0,u_t-e_t)$ lower-bounds the all-input fraction of newly induced errors. One paper supplied aligned values at the required granularity: 96 off-diagonal rates yield bounds from 1.2 to 77.9 points, median 34.5, mean 35.33, and interquartile range 23.55--45.35.

One released pipeline was adapted and rerun end to end on 1,000 images. Across its six targets with aligned published comparators, unconditional attacked-error deltas ranged from $-2.0$ to $+0.7$ points; pairwise-clean-correct PTR ranged from 45.69\% to 99.68\%, CTR from 45.84\% to 100\%, $a_{st}$ from 99.67\% to 99.68\%, and $b_{st}$ was 0 in every direction. Four other legacy workflows were audited but not completed, so their ledger cells remain blank.
