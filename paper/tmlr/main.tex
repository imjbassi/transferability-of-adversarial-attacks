...
\maketitle
\begin{abstract}
In a preregistered corpus of 80 highly cited papers, only 7 (8.75\%) explicitly restrict the transfer denominator to inputs clean-correct for the evaluated models, while 73 are unclear under a rule that treats silence as uncertainty rather than absence. Three papers (3.75\%) release code plus checkpoints or model artifacts sufficient to attempt recomputation; none releases directly reusable per-example outcomes. We connect these practices to the exact identity $\operatorname{PTR}_{st}=a_{st}\operatorname{CTR}_{st}+(1-a_{st})b_{st}$ and to the bound that unconditional attacked error minus target clean error lower-bounds newly induced error. Ninety-six rates in one qualifying paper yield bounds from 1.2 to 77.9 percentage points (median 34.5). A compact CIFAR-10 demonstration records every term of the identity. The finding concerns reporting sufficient to reconstruct CTR, never whether an individual paper is ``wrong.''
\end{abstract}

\section{Introduction}
Only 7 of 80 papers (8.75\%) in our preregistered corpus explicitly state that the transfer denominator excludes clean target errors. That percentage motivates the narrower question: what can a reader reconstruct from a reported transfer rate? Pairwise transfer rate and transfer conditional on source success are linked by the exact identity $\operatorname{PTR}_{st}=a_{st}\operatorname{CTR}_{st}+(1-a_{st})b_{st}$. Most judgments are \emph{unclear}, not negative; the result concerns reporting sufficient to reconstruct CTR.
...
Directionality and the tension between white-box optimization and transfer are established phenomena: \citet{demontis} explain asymmetric transfer through target vulnerability and surrogate complexity, while \citet{dong} use momentum to improve black-box transfer. We take this directional behavior as established and ask instead which population a reported transfer rate is conditioned on, and how far that choice moves the rate.
...
The corpus was frozen before coding at repository tag \texttt{survey-preregistration}. We queried Semantic Scholar for papers citing Liu et al. (arXiv:1611.02770) or Papernot et al. (arXiv:1602.02697), retained years 2018--2026 whose title or abstract contained ``transfer'' plus ``adversarial,'' ``black-box,'' or ``surrogate,'' sorted by citation count, and kept the first 80. The query, UTC run time, raw response, and ranked list are archived.

Each paper and supplement was coded yes/no/unclear for separate source/target clean accuracy; clean-correct transfer eligibility; source-success conditioning; absolute counts; a numerical $L_\infty$ or $L_2$ budget; and sufficient artifacts (code plus checkpoints, or per-example outputs). Silence is unclear, and ``evaluated on correctly classified images'' is yes. The planned delay was waived by explicit instruction after preregistration, so the seeded 16-paper second pass is an immediate repeatability check: all field-level and pooled Cohen's $\kappa$ values are 1.00, interpreted with the zero-day interval and common coder.

\section{Survey results}
Separate clean source/target accuracy was explicit in 5/80 papers (6.25\%); clean-correct transfer eligibility in 7/80 (8.75\%); source-success conditioning in 1/80 (1.25\%); and absolute counts in 0/80, with 57 percentage-only evaluations coded no and 23 unclear. A numerical norm budget was explicit in 46/80 (57.5\%). Three papers (3.75\%) met the artifact rule. These rates describe the preregistered corpus, including tangential mechanical-filter matches retained to avoid post hoc exclusion. An artifact audit records two initially plausible releases as unclear because the available checkpoints or evaluation path were insufficient to reconstruct the surveyed transfer results.

\section{Recomputation and analytic bounds}
The three qualifying releases were checked out at recorded commits and rerun on their released 1,000-image evaluation set. None supplies per-example clean/source/target predictions directly; we therefore adapted their aggregate evaluators, using an equation-preserving PyTorch adapter with the authors' linked converted checkpoints for the two TensorFlow~1 releases. Artifact availability (3.75\%) and immediately reusable per-example availability (0/80) are therefore distinct findings. For aligned target clean error $e_t$ and unconditional attacked error $u_t$, $\max(0,u_t-e_t)$ lower-bounds the all-input fraction of newly induced errors. One paper supplied aligned values at the required granularity: 96 off-diagonal rates yield bounds from 1.2 to 77.9 points, median 34.5, mean 35.33, and interquartile range 23.55--45.35.

All three qualifying releases were recomputed, yielding 27 source--target summaries and per-example prediction files. Across the 20 entries with aligned published comparators, unconditional attacked-error deltas ranged from $-3.9$ to $+5.5$ points (median $-0.65$); PTR ranged from 14.64\% to 100\%, CTR from 14.68\% to 100\%, and $a_{st}$ from 99.66\% to 100\%. The failed-source stratum was empty in 18 directions; in the nine directions where $b_{st}$ was defined it was 0. These are descriptive portability results, not claims of error.
...
\appendix
\section{CIFAR-10 controls moved from the main text}
The matched-learning-rate, FGSM, and convergence checks are retained in the machine-readable study artifacts and anonymous supplement. The 100-step endpoint changes off-diagonal PTR by at most 1.70 points at $8/255$ and by at most 0.27 points at $2/255$ on seed 0.
